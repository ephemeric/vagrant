...
DEBIAN_FRONTEND="noninteractive" apt-get -y --quiet install rpm >/dev/null

cd /home/vagrant/

#mkdir -p rpmbuild/{SPECS,BUILD,SRPMS,RPMS,SOURCES}

# Ubuntu is missing package systemd-rpm-macros. Required for `%{_unitdir}` in spec file.
cat >/usr/lib/rpm/macros.d/macros.systemd <<'EOF'
